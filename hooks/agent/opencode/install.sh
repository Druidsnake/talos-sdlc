#!/bin/sh
# Instala el enforcement de Thalos en el runtime de opencode.
#
...
PLUGIN_REL=".opencode/plugin/thalos-scope.js"
BRIEF_DST="AGENTS.md"
BACKUP="AGENTS.md.thalos-backup"
MARCA="GENERADO por Thalos"
...
if [ "${2:-}" = "--uninstall" ]; then
    ...
    if [ -f "$BRIEF_DST" ] && head -1 "$BRIEF_DST" | grep -q "$MARCA"; then
        ...
        # Lo que habia antes vuelve. Thalos toma prestado el archivo, no se lo
...
[ -f "$PLANTILLA" ] || { echo "thalos: falta la plantilla $PLANTILLA" >&2; exit 1; }
...
sed "s|__THALOS_SHIM__|$SHIM|" "$PLANTILLA" > "$PLUGIN_REL"

# El bloqueo tiene que quedar instalado de verdad, no intentado.
grep -q "$SHIM" "$PLUGIN_REL" || {
    echo "thalos: el plugin quedo sin la ruta del shim" >&2
    exit 1
}
...
if [ -n "$BRIEF" ] && [ -f "$BRIEF" ]; then
    if [ -f "$BRIEF_DST" ] && ! head -1 "$BRIEF_DST" | grep -q "$MARCA"; then
        # Hay un AGENTS.md que no es de Thalos: se guarda antes de tomar el
