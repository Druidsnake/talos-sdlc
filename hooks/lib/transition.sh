#!/bin/sh
# Ejecutor de transiciones. Ver thalos-0.0.7.md seccion 22.6.
#
# Hasta ahora Thalos sabia EVALUAR una transicion pero nada la EJECUTABA: el
...
#   22.6.2  una transicion no listada se rechaza y emite thalos.transition.rejected
...
#       thalos_transition_exec <maquina> <desde> <hacia> <dir-evidencia> [feature]

_tr_lib_dir=$(CDPATH='' cd -- "$(dirname -- "$0")" 2>/dev/null && pwd)
case "$_tr_lib_dir" in
    */lib) THALOS_TR_SYS=$(dirname "$(dirname "$_tr_lib_dir")") ;;
    *)     THALOS_TR_SYS="${THALOS_SYSTEM_ROOT:-$_tr_lib_dir}" ;;
esac
[ -n "${THALOS_SYSTEM_ROOT:-}" ] && THALOS_TR_SYS="$THALOS_SYSTEM_ROOT"

# shellcheck source=./gate.sh
. "$THALOS_TR_SYS/hooks/lib/gate.sh"

THALOS_LOCKS="${THALOS_LOCKS_FILE:-orchestration/locks.json}"

# thalos_feature_state <feature>  -> estado actual, o vacio si no existe
thalos_feature_state() {
    _sf="orchestration/features/$1/state.json"
    [ -f "$_sf" ] || return 1
    sed -n 's/.*"state"[[:space:]]*:[[:space:]]*"\([A-Z_]*\)".*/\1/p' "$_sf" | head -1
}

# thalos_feature_write <feature> <estado> <gate-result-ref> [seq]
# Proyeccion del event log, no fuente de verdad (seccion 41).
thalos_feature_write() {
    ...
    [ -n "${THALOS_ISSUE_REF:-}" ] && _issue="\"$THALOS_ISSUE_REF\""
    [ -n "${THALOS_BRANCH_REF:-}" ] && _branch="\"$THALOS_BRANCH_REF\""
    [ -n "${THALOS_LEASE_ID:-}" ] && _leases="\"$THALOS_LEASE_ID\""
    ...
    cat >"$_dir/state.json" <<EOF
{
  "schema_version": 1,
  "feature_id": "$_f",
  "run_id": "${THALOS_RUN_ID:-r-unknown}",
  "state": "$_st",
  "issue_ref": $_issue,
  "branch_ref": $_branch,
  "pr_ref": null,
...
# thalos_mint_lock_release <feature> <lease_id>
...
{"id":"$(basename "$_ev")","kind":"LockRelease","schema_version":1,
 "run_id":"${THALOS_RUN_ID:-r-unknown}","feature_id":"$1",
 "produced_by":"core:LockManager","produced_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ)",
 "digest":"pendiente","verifiable":true,"payload":{"lease_id":"$2"}}
EOF
    "$_py" "$THALOS_TR_SYS/hooks/lib/evidence.py" seal "$_ev.json" >/dev/null 2>&1
    printf '%s' "$_ev.json"
}

# thalos_release_feature_leases <feature>
# Regla 22.6.8: al alcanzar un estado terminal se liberan todos los leases.
thalos_release_feature_leases() {
    _py=$(thalos_python) || return 0
    [ -f "$THALOS_LOCKS" ] || return 0
    "$_py" "$THALOS_TR_SYS/hooks/lib/lock.py" list "$THALOS_LOCKS" 2>/dev/null \
        | awk -F'\t' -v f="$1" '$3 == f { print $1 }' \
        | while read -r _lid; do
            [ -z "$_lid" ] && continue
            "$_py" "$THALOS_TR_SYS/hooks/lib/lock.py" release "$THALOS_LOCKS" "$_lid" >/dev/null 2>&1
            thalos_mint_lock_release "$1" "$_lid" >/dev/null 2>&1 || true
            printf '%s\n' "$_lid"
        done
}
...
thalos_transition_exec() {
    _m="$1"; _from="$2"; _to="$3"; _evdir="$4"; _feat="${5:-}"

    _cli="$THALOS_TR_SYS/cli/thalos"
    _event=$(thalos_transition_event "$_m" "$_from" "$_to" 2>/dev/null || echo "")
    _actor=$(thalos_transition_actor "$_m" "$_from" "$_to" 2>/dev/null || echo "Orchestrator")

    set +e
    _result=$(thalos_gate_eval "$_m" "$_from" "$_to" "$_evdir" "${THALOS_RUN_ID:-}" "$_feat")
    _code=$?
    set -e

    # Regla 22.6.6: el GateResult que autoriza -o rechaza- queda registrado.
    _saved=$(thalos_gate_persist "$_result" "orchestration/evidence" \
                "${THALOS_RUN_ID:-}" "$_feat" 2>/dev/null || true)
    ...
    if [ "$_code" -ne 0 ]; then
        ...
        _rej_args="--type thalos.transition.rejected --actor core:GateEvaluator"
        ...
        echo "event=thalos.transition.rejected"
        return "$_code"
    fi
    ...
    if [ "$_m" = feature ] && [ -n "$_feat" ]; then
        thalos_feature_write "$_feat" "$_to" "$_ev_id" "$_seq"
        if thalos_is_terminal feature "$_to"; then
            thalos_release_feature_leases "$_feat" | while read -r _l; do
