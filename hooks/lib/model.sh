#!/bin/sh
# Resolucion capability_tier -> modelo concreto. Ver thalos-0.0.7.md 20.3.
#
...
#       thalos_model_for_tier fast     -> "<modelo>\t<proveedor>"
#       thalos_tier_of_feature F001    -> "fast"

# thalos_model_for_tier <tier>
# Emite "<modelo>\t<proveedor>". Sale 1 si no hay tabla o el tier no esta.
thalos_model_for_tier() {
    _mt_py=$(thalos_python 2>/dev/null) || return 1
    _mt_root="${THALOS_PROJECT_ROOT:-.}"
    _mt_sys="${THALOS_SYSTEM_ROOT:-$_mt_root}"
    for _mt_f in "$_mt_root/config/models.yaml" \
                 "$_mt_root/.thalos/config/models.yaml" \
...
thalos_role_minimum_tier() {
    _rm_py=$(thalos_python 2>/dev/null) || return 1
    _rm_root="${THALOS_PROJECT_ROOT:-.}"
    _rm_sys="${THALOS_SYSTEM_ROOT:-$_rm_root}"
    for _rm_f in "$_rm_sys/config/roles.yaml" \
                 "$_rm_root/.thalos/config/roles.yaml" \
...
# thalos_tier_resolve <feature_id> <rol>
#
...
thalos_tier_resolve() {
    _tr_feat=$(thalos_tier_of_feature "$1" 2>/dev/null || echo "")
    _tr_rol=""
    [ -n "${2:-}" ] && _tr_rol=$(thalos_role_minimum_tier "$2" 2>/dev/null || echo "")
...
# thalos_tier_of_feature <feature_id>
...
# de por riesgo.
thalos_tier_of_feature() {
    _tf_py=$(thalos_python 2>/dev/null) || return 1
    _tf_plan="${THALOS_PROJECT_ROOT:-.}/orchestration/program-plan.json"
    [ -f "$_tf_plan" ] || return 1
