...
#       talos_merge_gate <feature_id> <pr> <dir-evidencia>

_mg_lib_dir=$(CDPATH='' cd -- "$(dirname -- "$0")" 2>/dev/null && pwd)
case "$_mg_lib_dir" in
    */lib) TALOS_MG_SYS=$(dirname "$(dirname "$_mg_lib_dir")") ;;
    *)     TALOS_MG_SYS="${TALOS_SYSTEM_ROOT:-$_mg_lib_dir}" ;;
esac
[ -n "${TALOS_SYSTEM_ROOT:-}" ] && TALOS_MG_SYS="$TALOS_SYSTEM_ROOT"

# shellcheck source=./gate.sh
. "$TALOS_MG_SYS/hooks/lib/gate.sh"
# shellcheck source=./resolve-capability.sh
. "$TALOS_MG_SYS/hooks/lib/resolve-capability.sh"

TALOS_MG_LOCKS="${TALOS_LOCKS_FILE:-orchestration/locks.json}"

_mg_config() {
    grep -E "^$1:" "$TALOS_MG_SYS/config/system.yaml" 2>/dev/null \
        | head -1 | sed "s/$1:[[:space:]]*//" | tr -d '"'
}

# talos_feature_risk <feature_id>
...
talos_feature_risk() {
    _plan="${TALOS_PROJECT_ROOT:-.}/orchestration/program-plan.json"
    [ -f "$_plan" ] || { printf 'critical'; return 0; }
    _py=$(talos_python) || { printf 'critical'; return 0; }
...
talos_merge_gate() {
    ...
    _mode=$(talos_execution_mode)
    _risk=$(talos_feature_risk "$_feat")
    ...
    set +e
    _checks=$(talos_capability_run CIAdapter get_check_status "{\"pr\":\"$_pr\"}" 2>&1)
    _crc=$?
    set -e
    if [ "$_crc" -ne 0 ]; then
    ...
    set +e
    _mergeable=$(talos_capability_run CoordinationAdapter get_pr_checks "{\"pr\":\"$_pr\"}" 2>&1)
    _mrc=$?
    set -e
    if [ "$_mrc" -ne 0 ]; then
    ...
    _base="branch:main"
    if [ -f "$TALOS_MG_LOCKS" ] && _py=$(talos_python); then
        _owner=$("$_py" "$TALOS_MG_SYS/hooks/lib/lock.py" list "$TALOS_MG_LOCKS" 2>/dev/null \
    ...
    if [ "$_risk" = critical ] || [ "$_hp" = always ]; then
        if talos_evidence_kinds "$_evdir" | grep -qx HumanApproval; then
    ...
    printf '"execution_mode":"%s","evaluated_at":"%s","evaluator_version":"%s"}\n' \
        "$_mode" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "${TALOS_VERSION:-0.0.6}"
