...
## 3. Alcance de la versión 0.0.5

La versión 0.0.5 define:
...
## 4. No alcance de la versión 0.0.5

La versión 0.0.5 NO define completamente:
...
La versión 0.0.5 está orientada a:
...
7. Auto-merge en v0.0.5 DEBE estar deshabilitado por defecto.
...
3. v0.0.5 RECOMIENDA `max_parallel_features = 1`.
...
5. v0.0.5 RECOMIENDA límites estrictos.
...
## 46. Criterios de aceptación de Talos v0.0.5

La versión 0.0.5 se considera aceptable si:
...
35. El sistema opera solo con el adapter dry-run.
36. El sistema opera sin ninguna extensión opcional habilitada.
37. Ninguna extensión aprueba gates ni produce evidencia de transición.

---

## 47. Modo recomendado para v0.0.5
...
plugins = optional
memory_extension = optional
routing = capability_based
```

---

## 48. Migración desde 0.0.4

### 48.1. Cambios que rompen compatibilidad
...
La versión 0.0.5 NO DEBE extenderse con nuevas secciones normativas antes de tener un vertical slice ejecutable.
...
4. DryRunAdapter                 (sección 38)
5. talos doctor / spec check     (secciones 27, 28)
6. talos plan                    (sección 29)
7. talos feature start (dry-run) (sección 30)
8. LockManager                   (sección 32)
9. GitHub + CI adapters reales   (sección 38)
10. MergeGate                    (sección 31)
```

Las extensiones opcionales, incluida la memoria, NO DEBEN implementarse antes del paso 10.
