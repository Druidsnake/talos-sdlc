#!/bin/sh
# thalos.adapter.github - implementacion productiva de CoordinationAdapter.
#
# Uso:   run.sh <operacion> [semantic_args_json]
# Env:   THALOS_RUN_ID, THALOS_FEATURE_ID   contexto de la idempotency key
#        THALOS_GH_BIN                     primer paso de la cascada (37.4.5)
#        THALOS_DRY_RUN=1                  registra la intencion, no toca el remoto
...
#   Thalos volveria a crearlo. El remoto es la fuente de verdad de lo que existe
#   en el remoto.
...
gh_do() {
    if [ "$DRY" = 1 ]; then
        thalos_ledger_record "dryrun-$(date -u +%s)-$$" "gh:$1" "{\"intended\":\"$*\"}"
...
thalos_marker() {
    printf '<!-- thalos-idempotency-key: %s -->' "$1"
}
...
THALOS_GH_SCAN="${THALOS_GH_SCAN:-100}"

busca_issue() {
    [ "$DRY" = 1 ] && return 1
    "$GH" issue list --state all --json number,body --limit "$THALOS_GH_SCAN" 2>/dev/null \
        | tr '}' '\n' \
        | grep -F "$1" \
...
busca_pr() {
    [ "$DRY" = 1 ] && return 1
    "$GH" pr list --state all --json number,body --limit "$THALOS_GH_SCAN" 2>/dev/null \
        | tr '}' '\n' \
        | grep -F "$1" \
        | sed -n 's/.*"number":\([0-9]*\).*/\1/p' | head -1
}
...
case "$op" in
    health)
        ...
        if [ "$DRY" = 1 ]; then
            thalos_ok "{\"healthy\":true,\"capability\":\"CoordinationAdapter\",\"version\":\"$_v\",\"path\":\"$GH\",\"dry_run\":true}"
            ...
            thalos_ok "{\"healthy\":true,\"capability\":\"CoordinationAdapter\",\"version\":\"$_v\",\"path\":\"$GH\",\"repo\":\"$_repo\"}"
        else
            printf '{"status":"error","error_class":"precondition",' >&2
    ...
    create_issue)
        ...
        [ -n "$_title" ] || { thalos_error precondition "create_issue requiere title"; exit 5; }
        ...
        _out=$(gh_do issue create --title "$_title" \
               --body "$(printf '%s\n\n%s' "${_body:-Creado por Thalos para $feat}" "$(thalos_marker "$_key")")") || {
        ...
        thalos_ledger_record "$_key" "$op" "{\"id\":\"$(basename "${_url:-0}")\",\"url\":\"$_url\"}"
    ...
    create_branch)
        check_version >/dev/null || exit 2
        _name=$(json_get name)
        [ -n "$_name" ] || { thalos_error precondition "create_branch requiere name"; exit 5; }
        _key=$(thalos_idempotency_key "$run" "$feat" "$op" "$args") || exit 5
        ...
        if [ "$DRY" = 1 ]; then
            thalos_ledger_record "$_key" "$op" "{\"id\":\"$_name\",\"url\":null}"
    ...
    open_pr)
        ...
        [ -n "$_title" ] && [ -n "$_head" ] || {
            thalos_error precondition "open_pr requiere title y head"; exit 5; }
        _key=$(thalos_idempotency_key "$run" "$feat" "$op" "$args") || exit 5
        ...
        _out=$(gh_do pr create --title "$_title" --head "$_head" \
               ${_base:+--base "$_base"} \
               --body "$(printf 'Abierto por Thalos para %s\n\n%s' "$feat" "$(thalos_marker "$_key")")") || {
    ...
    get_pr_checks)
        check_version >/dev/null || exit 2
        _pr=$(json_get pr)
        [ -n "$_pr" ] || { thalos_error precondition "get_pr_checks requiere pr"; exit 5; }
        ...
        thalos_ok "{\"pr\":\"$_pr\",\"checks\":$(printf '%s' "$_out" | grep -q '^\[' && printf '%s' "$_out" || printf '[]')}"
        ;;

    request_review)
        check_version >/dev/null || exit 2
        _pr=$(json_get pr); _who=$(json_get reviewer)
        [ -n "$_pr" ] || { thalos_error precondition "request_review requiere pr"; exit 5; }
        thalos_mutate_run "$op" "$run" "$feat" "$args" number \
